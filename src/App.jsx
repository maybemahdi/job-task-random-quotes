    ...
    <>
      <div className="pt-20 min-h-screen font-main text-white bg-gradient-to-r from-black to-gray-500">
        <div className="flex flex-col items-center justify-center">
          <h2 className="md:text-6xl text-4xl font-bold text-center">Random Quotes</h2>
          <div className="flex flex-col my-10">
            <div className="overflow-hidden px-8 rounded-3xl text-2xl font-bold">
              {loading ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6 text-center text-black">
                  {quotes.map((quote, idx) => (
                    <div
                      className="flex flex-col rounded bg-purple-400 hover:bg-purple-500 transition-all duration-500"
                      key={idx}
                    >
                      <h3 className="w-[90%] h-[90%] my-10 mx-auto font-local">
                        `{quote}`
                      </h3>
                      <button
                        onClick={() => handleSave(quote)}
                        className="mb-5 mx-6 bg-pink-500 transition-all duration-500 hover:bg-purple-400 shadow-2xl p-2 rounded-full"
                      >
                        Save to List
                      </button>
                    </div>
             ...
             {!loading && (
                <button onClick={handleGenerate} className="w-40 h-12 bg-white cursor-pointer rounded-3xl border-2 border-[#9748FF] shadow-[inset_0px_-2px_0px_1px_#9748FF] group hover:bg-[#9748FF] transition duration-300 ease-in-out">
